...
- **Wizards**: `GUI/Wizards/WizardBase.h` — multi-step overlay with progress dots, back/next/skip navigation. Subclasses (`StartWizard`, `AudioSetupWizard`, `PresetFinderWizard`) override `buildStepContent()` and `onStepChanged()`. Shown as full-screen modal overlays. Controls are children of the per-step page component, so `resized()` must lay them out in **page-local** coordinates (`getContentArea().withTrimmedTop(60).withZeroOrigin()`); wizard coordinates offset them by the page origin and push them out of view.

- **User feedback** (`Core/Audio/UserAdvisor.h` + `GUI/Widgets/AdviceOverlay.h`): the advisor turns engine telemetry into plain-language advice — no buffer sizes, sample rates, CPU percentages or "dropout"/"xrun" in any user-visible string, which a unit test enforces against a banned-word list. Advice is raised on the *transition* into a condition and cannot repeat until it has cleared for `rearmSeconds`, so a machine sitting just over a threshold does not nag. `PluginEditor::updateAdvice()` feeds it a snapshot from the existing 30 Hz stats poll; the overlay draws non-modal cards whose `hitTest()` claims only their own pixels, so the pedalboard stays usable underneath.

- **Audio auto-configuration** (standalone only, `Core/Audio/`): `AudioAutoConfig` ranks the available back-ends (ASIO > WASAPI exclusive > WASAPI low-latency > shared > DirectSound; CoreAudio on macOS), probes the device for its real sample rates and buffer sizes, and targets 48 kHz — the saturation pedals already oversample internally, so a higher device rate costs CPU without adding quality. `LatencyCalibration` then measures what the machine actually sustains: it reopens the device at each candidate buffer, smallest first, and accepts the first that runs with zero `getXRunCount()` dropouts and peak `getCpuUsage()` under 70%. The measured floor is a hard lower bound; the use-case profile only chooses how much margin to keep above it. Both surface *why* each value was chosen, so the wizard can name the constraint that sets the user's ceiling. The sweep must run with the engine on — a bypassed graph reports a floor the real chain cannot hold.
